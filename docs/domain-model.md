...
| Session | id, userId, expiresAt, revokedAt, revokedReason | One per login; revocation is explicit and recorded with a reason; `expiresAt` is enforced | 2 †|
...
**The session is a hard cap, and the column is enforced rather than bookkeeping.** Rotation renews the
refresh token, not the session, so a client that refreshes indefinitely still has to authenticate
again once the session's own expiry passes. `decideRefreshTokenUse` refuses a fresh token in an expired
session — an earlier version selected the column and never read it, which made it decoration.
